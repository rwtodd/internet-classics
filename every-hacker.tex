...
poor enough \acro{RF} shielding) that {\bf this} was common knowledge: you
...
The fragility of the {\tt 0x80}~bit back in those old days is the now largely
...
What the \acro{VDT} era {\bf does} explain is some of our heritage games (see next section)
...
Because hackers {\bf needed} good text display for programming --- and because many
...
\medbreak
\dlist \acro{NUL} (Null) = Ctrl-@. Survives as the string terminator in \CEE/.
...
least some pre-\acro{ANSI} ones interpreted \acro{VT} as ``scroll {\bf up} one line.'' The only
...
soon after \acro{VDT}s replaced teletypes) is that the binary value of \acro{DEL} ({\tt 0x7F},
...
access to source code. In the \acro{BBS} world, FidoNet is invented. The {\tt patch(1)}
utility is invented.
