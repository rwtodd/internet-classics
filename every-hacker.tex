...
\usepackage[letterpaper,total={5.6in,8.5in}]{geometry}
\usepackage[open,openlevel=1]{bookmark}

\newenvironment{keydatelist}%
{\begin{list}{}{%
\renewcommand{\makelabel}[1]{\hspace\labelsep\textbf{##1}\hfil}%
\settowidth{\labelwidth}{\hspace\labelsep\textbf{9999\,}}%
...
modem'' cables, which for reasons too painful to go into here cross-wired the
...
what's called ``\acro{TTL} serial'' --- same signaling with a swing of 3.3 or (less
...
\section[WAN Time Gone: The Forgotten Pre-Internets]{WAN Time Gone: The Forgotten\\ Pre-Internets}
...
Meanwhile, a different kind of store-and-forward
evolved in the microcomputer world --- the \acro{BBS} (Bulletin-Board System). This was software running on a
...
one user at a time!) would access the \acro{BBS} using their own modem and a
...
\vspace{6ex}
\begin{center}
\leavevmode\vbox{\scriptsize\ttfamily\offinterlineskip
\halign{%
\hfil#&\hskip0.5em\hfil#&\hskip0.5em#\hfil&\hskip0.38em\vrule height2.1ex depth1.1ex \hskip0.38em%
...
\omit\hskip 0.38em\vrule height2.1ex depth1.1ex\hskip 0.38em\hfil Dec&\omit\ Hex&&%
...
\leavevmode\vbox{\scriptsize\ttfamily\offinterlineskip
\halign{%
#\hfil&\quad#\hfil&\hskip1.5em\vrule height 2.1ex depth1.1ex\hskip1.5em#\hfil&\quad#\hfil%
...
\begin{description}
\item[\acro{NUL} (Null) = Ctrl-@]\hfill\\ Survives as the string terminator in \CEE/.
\item[\acro{SOH} (Start of Heading) = Ctrl-A]\hfill\\ Rarely used (as Ctrl-A) as a section
...
something similar.
\item[\acro{STX} (Start of Text), \acro{ETX} (End of Text) = Ctrl-B, Ctrl-C]\hfill\\ Very rarely
...
\item[\acro{EOT} (End of Transmission) = Ctrl-D]\hfill\\ As Ctrl-D, the way you type ``End of
file'' to a \UNIX/ terminal.
\item[\acro{ENQ} (Enquiry) = Ctrl-E]\hfill\\ In the days of hardware serial terminals, there
...
least gave computers a fighting chance of auto-configuring what capabilities it
...
are you?'').
\item[\acro{ACK} (Acknowledge) = Ctrl-F]\hfill\\
...
Cat's ``Ack! Thppt!''
\item[\acro{BEL} (Bell) = Ctrl-G]\hfill\\
...
\item[\acro{BS} (Backspace) = Ctrl-H]\hfill\\
...
\item[\acro{HT} (Horizontal tab) = Ctrl-I]\hfill\\
...
\item[\acro{LF} (Line Feed) = Ctrl-J]\hfill\\
...
\item[\acro{VT} (Vertical Tab) = Ctrl-K]\hfill\\
...
definitions of what a white-space character is, even though it's now extinct in
the wild.
\item[\acro{FF} (Form Feed) = Ctrl-L]\hfill\\ Eject the current page from your printing
...
\item[\acro{CR} (Carriage Return) = Ctrl-M]\hfill\\
...
\item[\acro{SO} (Shift Out), \acro{SI} (Shift In) = Ctrl-N, Ctrl-O]\hfill\\
...
shift back.
\item[\acro{DLE} (Data Link Escape) = Ctrl-P]\hfill\\
...
\item[\acro{DC\char`\[1234\char`\]} (Device Control \char`\[1234\char`\]) = Ctrl-\acro{\char`\[QRST\char`\]}]%
\hfill\\ 
...
too.
\item[\acro{NAK} (Negative Acknowledge) = Ctrl-U]\hfill\\ See the discussion of \acro{ACK} above.
\item[\acro{SYN} (Synchronous Idle) = Ctrl-V]\hfill\\
...
\item[\acro{ETB} (End of Transmission Block) = Ctrl-W]\hfill\\
...
\item[\acro{CAN} (Cancel), \acro{EM} (End of Medium) = Ctrl-X, Ctrl-Y]\hfill\\
Never to my knowledge used specially after teletypes.
\item[\acro{SUB} (Substitute) = Ctrl-Z]\hfill\\
...
\item[\acro{\char`\[FGRU\char`\]S} ($\lbrace$Field$\vert$Group$\vert$Record$\vert$Unit$\rbrace$ Separator)]\hfill\\
...
\item \acro{VDT} keyboards often had a ``Break'' key inherited from the \acro{ASR-33} (there is a
...
\section[The Slow Birth of Distributed Collaboration]{The Slow Birth of Distributed\\ Collaboration}
...
\item[1992] Bitmapped color displays with a dot pitch matching that of monochrome \acro{VDT}s
...
\item[1995--1996]\mbox{}\\ Peak years of \acro{UUCP/USENET} and the \acro{BBS}
culture, then collapse under pressure from mass-market Internet.
...
\item[2007--2008]\mbox{}\\ 64-bit transition in mass market \acro{PC}s; the
32-bit era ends. Processor speeds plateau at $4\pm0.25$GHz. iPhone and
Android (both with \UNIX/-based OSes) first ship.
\end{keydatelist}
\end{document}
