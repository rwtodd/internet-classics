...
\quotation{When you have learned to snatch the error code from the trap
...
\book{4}{Coding}
Thus spake the Master Programmer:
\quotation{A well-written program is its own Heaven; a poorly-written
...
\book{5}{Maintenance}
Thus spake the Master Programmer:
\quotation{Though a program be but three lines long, someday it will
have to be maintained.}

\vsect{5.1} A well-used door needs no oil on its hinges.
A swift-flowing stream does not grow stagnant.  A deer blends
perfectly into the forest colors.  Software rots if not used.
\endverse
...
\book{6}{Management}
Thus spake the Master Programmer:
\quotation{Let the programmers be many and the managers few---then
...
Thus spake the Master Programmer:
\quotation{You can demonstrate a program for a corporate executive,
...
Thus spake the Master Programmer:
\quotation{Without the wind, the grass does not move. Without software,
hardware is useless.}
